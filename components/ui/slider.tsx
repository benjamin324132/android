...
>(({ className, ...props }, ref) => (
  <SliderPrimitive.Root
    ...
    {...props}
  >
    <SliderPrimitive.Track className="relative h-12 w-full grow overflow-hidden rounded-xl bg-secondary">
      <SliderPrimitive.Range className="absolute h-full bg-primary rounded-tr-xl rounded-br-xl" />
    </SliderPrimitive.Track>
    <SliderPrimitive.Thumb className="block h-7 w-[0.15rem] rounded-sm mr-5 border-0 border-primary bg-background  transition-colors focus-visible:outline-none  disabled:pointer-events-none disabled:opacity-50" />
  </SliderPrimitive.Root>
));
